...
void ULagCompensationComponent::StoreFramePackage()
{
	if (!IsValid(Character) || !Character->HasAuthority()) return;
	/**
	* store all the characters on the server whether locally control or not
	*/
...
{
	Character = IsValid(Character) ? Character : Cast<ABlasterCharacter>(GetOwner());
	if (Character)
	{
		Package.Time = GetWorld()->GetTimeSeconds();
		Package.Character = Character; // get the character who use the lagcompensation,right?
...
FServerSideRewindResult ULagCompensationComponent::ConfirmHit(const FFramePackage& Package, ABlasterCharacter* HitCharacter, const FVector_NetQuantize& TraceStart, const FVector_NetQuantize& HitLocation)
{
	...
	MoveBoxes(HitCharacter, Package); // take the HitCharacter to the past 
	...
	ResetHitBoxes(HitCharacter, CurrentFrame); // bring HitCharacter back.
	EnableCharacterMeshCollision(HitCharacter, ECollisionEnabled::QueryAndPhysics);
...
FShotgunServerSideRewindResult ULagCompensationComponent::ShotgunConfirmHit(const TArray<FFramePackage>& FramePackages, const FVector_NetQuantize& TraceStart, const TArray<FVector_NetQuantize>& HitLocations)
{
	...
	for (auto& Frame : FramePackages)
	{
		...
		UBoxComponent* HeadBox = Frame.Character->HitCollisionBoxes[FName("head")];
		HeadBox->SetCollisionEnabled(ECollisionEnabled::NoCollision); // disable for head box
	}
...
void ULagCompensationComponent::ResetHitBoxes(ABlasterCharacter* HitCharacter, const FFramePackage& Package)
{
	...
	for (auto& HitBoxPair : HitCharacter->HitCollisionBoxes)
	{
		if (HitBoxPair.Value != nullptr)
		{
			...
			HitBoxPair.Value->SetWorldRotation(Package.HitBoxInfo[HitBoxPair.Key].Rotation);
			HitBoxPair.Value->SetBoxExtent(Package.HitBoxInfo[HitBoxPair.Key].BoxExtent);
			HitBoxPair.Value->SetCollisionEnabled(ECollisionEnabled::NoCollision); // this is the point,we disable the boxcomponent collision there.
		}
	}
}
...
{
	bool bReturn = !IsValid(HitCharacter) ||
		!IsValid(HitCharacter->GetLagCompensation()) ||
...
void ULagCompensationComponent::ShotgunServerScoreRequest_Implementation(const TArray<ABlasterCharacter*>& HitCharacters, const FVector_NetQuantize& TraceStart, const TArray<FVector_NetQuantize>& HitLocations, float HitTime)
{
	...
	for (auto& HitCharacter : HitCharacters)
	{
		if (!IsValid(HitCharacter) || !IsValid(HitCharacter->GetEquippedWeapon())|| !IsValid(Character)) continue;
		...
		UGameplayStatics::ApplyDamage(
			HitCharacter,
			TotalDamage,
			Character->GetController(), // Instigator Controller.
